...
\begin{tabular}{c|l|l}
    ...
    网页性能优化 & YSlow & 3.1.8.1\\		\end{tabular}
...
浏览此文件时会有一个网址，形如：\url{http://119.29.81.128:8018/WCFService/Tools/UploadFile.svc?wsdl}，
...
<system.serviceModel>
    ...
    <client>
        <endpoint address="http://119.29.81.128:8018/WCFService/Tools/UploadFile.svc?wsdl"
...
\url{http://119.29.81.128:8019/WCFService/Product/PublicProductService.svc/
...
\begin{lstlisting}
同源策略禁止读取位于http://119.29.81.128:8019/Product/web/Get?{%20%22a%22:%20%22a%22%20}&_=1441558531991 的远程资源。（原因：CORS 头缺少 'Access-Control-Allow-Origin'）。
\end{lstlisting}
...
$(function () {
    $("#input-verifycros").bind("click", function () {               
        ...
        var url = "http://119.29.81.128:7019/WCFService/Custom/ServiceCros.svc/web/DoWork/a";                
        if (xmlHttpRequest) {
...
<script>
    $(function () {
        $("#input-verify").bind("click", function () {                 
            ...
            var url = "http://119.29.81.128:8019/Product/web/Get/a";
            jQuery.ajax({                    
                url: url,                                       
                type: 'get',                    
                dataType: 'jsonp', 
...
public JpegFileInfoResult UploadJpegImage(Stream fileStream)
{
    ...
    try
    {
        ...
        {
            storePath = System.IO.Path.Combine(HttpRuntime.AppDomainAppPath, "\\upload\\");
        }
...
\begin{lstlisting}
<add name="WCFCredential" connectionString="Data Source=119.29.81.128,5002;Uid=WCFCredential;Pwd=WCFCredential!@#321;database=WCFCredential"/>
\end{lstlisting}
...
\subsection{Automated Documentation for REST APIs}
...
or through network traffic inspection. When properly defined via Swagger, 
...
#region 创建二维码-混合平台（Web+WinForm）
/// <summary>
/// 
/// </summary>
/// <param name="data">二维码内容</param>
/// <param name="qrCodeEncoder">Encoder实例</param>
/// <returns></returns>
public string CreateQRCodeWithRelativePath(string data, QRCodeEncoder qrCodeEncoder)
{
    ValidateUtil.RequireNotNullOrEmpty("data", data);
    string fileName = string.Empty;
    string tempPath = string.Empty;
    string QRCodeStorePath = string.Empty;
    string relativePath = string.Empty;
    try
    {
        fileName = System.Guid.NewGuid().ToString() + ".png";
        QRCodeStorePath = ConfigUtil.GetConfigString("QRCodeStorePath");
        relativePath = (QRCodeStorePath.EndsWith("/") ? QRCodeStorePath : (QRCodeStorePath + "/"))`\makeremark{此处生成的url路径需要用/，而不是\textbackslash，        这点很重要，因为Firefox对路径要求比较严格，生成的二维码url路径如果为\textbackslash ，在Firefox浏览器中可能无法解析此url路径。}`;
        if (HttpContext.Current != null)
        {
            tempPath = HttpContext.Current.Server.MapPath("/") + relativePath;`\makeremark{此处在获取服务器上面的相对路径时使用了HttpContext.Current.Server.MapPath的方式。}`
        }
        else
        {
            tempPath = Application.StartupPath + relativePath;
        }
    ...
    {
        logger.Error("Generate QRCode encount an error", e);
    }
    return domainName + "/" + relativePath + fileName;
}
#endregion
\end{lstlisting}
...
程序有时需要动态的配置项，如字典类型的配置(Key-Value)，
一般的序列化方法不支持字典类型的序列化，
...
\subsubsection{将DataTable转为List}
...
如果转型失败，则要将值设定为一个初始值，同时必须要捕获异常，代码如下所示：
...
\subsubsection{List与string的转换}

我们在开发中经常会用List<string>来保存一组字符串，
比如下面这段代码：
...
\subsubsection{Chrome Javascript条件断点(Condition Breakpoint)}
...
这(Expires)应该是WEB中最早的缓存机制了，是在HTTP协议中实现的，有点不同于Dom Storage、AppCache 等缓存机制，
但本质上是一样的。可以理解为，一个是协议层实现的，一个是应用层实现的。

\subsection{Request缓存相关首部字段}

\paragraph{Cache-Control}Cache-Control在HTTP 1.1中加入的，
...
\paragraph{POST传值（View \textrightarrow ~Controller)}
...
\subsubsection{Application\_Error}
...
\begin{lstlisting}[language={[Sharp]C},caption=应用程序级别错误处理,label=code:ApplicationLevelErrorHandler]
void Application_Error(object sender, EventArgs e)
{
    ...
    var ip = (serverIp != null) ? serverIp.Trim() : remoteAddr;
    str.Append("\r\n\tIp:" + ip);
    ...
    Server.ClearError();
    //跳转至友好的出错页面 
    //Response.Redirect("~/Views/Error/error.html");
}
\end{lstlisting}
...
\subsubsection{OnException继承HandleErrorAttribute}
...
另ViewData和ViewBag的简单对比如表\ref{dd}所示：

\begin{table}\caption[Caption for LOF]{修改记录}\label{dd}					
	\medskip
	\centering		
	\begin{tabular}{|c|c|}
		\hline
...
function saveUserPhone(mobile) {
    ...
    xhr.open("GET", "/Active/SaveUserPhone?ccnId=" + cardId + "&phone=" + mobile, true);
    xhr.send(null);
}
\end{lstlisting}
...
insert openquery ([119.21.80.128,5002], ' SELECT * FROM [renren-weixin].[dbo].[tb_wxUser1] ' ) 
select * from [zouwo-weixin].[dbo].tb_wxUser

--将远程表导入本地表
select * into tb_awardsCopy from openrowset('SQLOLEDB ', '119.21.80.128,5002'; 'renren-admin'; 'renren-admin!@#','select * from [renren-system].[dbo].[tb_awards]')
...
\begin{itemize}
\item{-F}扫描100个最有可能开放的端口,F表示Fast Model，
比默认模式扫描更少的端口（Scan fewer ports than the default scan），
如果需要扫描更多的端口哦，指定p参数，如下代码片段所示：
...
\item{-v}获取扫描的信息
\item{-sT}采用的是TCP扫描，不写也是可以的，默认采用的就是TCP扫描
\end{itemize}
...
var config = {
    user: "renren-admin",
    password: "renren-admn!@#321",
    server: "119.29.81.129", // You can use "localhost\\instance" to connect to named instance
    port:"5001",
...
\begin{lstlisting}[language={[Sharp]C}]
Message<List<tb_usersModel>> msg = _apiClient.GetModel<List<tb_usersModel>>("http://119.29.81.128:8018/User/web/GetUserInfoByID?userId=888");
...
在vista,server2003,xp sp2中这个值为1，在xp,2000 sp4中为0。1值时的搜素顺序为：
...
而在.NET下，这个过程却不大一样。用PE结构查看工具（这里使用PEiD）载入一个实例文件，观察导入表，可以发现整个表只引入了一个mscoree.dll中的一个方法：
\_CorExeMain。而这个方法，真是该可执行文件在win32意义上的入口点。
...
2、PE loader通过查找CLR头发现该目录不为空，则自动将mscoree.dll载入进程地址空间中，
...
4、\_CorExeMain方法会帮助程序找到并载入适当的CLR版本，
在.net 2.0以后实现CLR的程序集为mscorwks.dll或mscorsvr.dll，例如，
在我的机器上mscorwks.dll的位置是：C:/WINDOWS/Microsoft.NET/Framework/v2.0.50727/；
...
\begin{itemize}
\item{Use comparisons with StringComparison.Ordinal or StringComparison.OrdinalIgnoreCase for better performance.}
...
<configuration xmlns:xdt="http://schemas.microsoft.com/XML-Document-Transform">
  <appSettings>
    <!-- WCF正式接口路径 -->
    <add key="WCFPath" value="http://119.29.81.128:8888" xdt:Transform="SetAttributes" xdt:Locator="Match(key)"/>`\makeremark{设置（SetAttributes）生产环境所调用的接口的值，根据key进行匹配（Match(key)）}`
    <!--新版静态文件地址-->
...
\textbf{lockingModel}属性为文件锁类型，RollingFileAppender本身不是线程安全的，如果在程序中没有进行线程安全的限制，
可以在这里进行配置，确保写入时的安全。文件锁定的模式，官方文档上他有三个可选值“FileAppender.ExclusiveLock, 
FileAppender.MinimalLock and FileAppender.InterProcessLock”，默认是第一个值，排他锁定，
一次值能有一个进程访问文件，close后另外一个进程才可以访问；第二个是最小锁定模式，允许多个进程可以同时写入一个文件。
...
#生成密钥,在github.com的网站上到ssh密钥管理页面，添加新公钥，随便取个名字，内容粘贴生成的公钥
...
<sourcecontrol 	type="svn">
	<!--该项目的SVN路径-->
    <trunkUrl>https://119.29.81.128/svn/hotmark</trunkUrl>  
...
\begin{lstlisting}[language={[Sharp]C}]
if(oSession.host.Contains("192.168.1.222:8101")||oSession.host.Contains("119.29.81.128:8101"))
{
...
\begin{lstlisting}
Provider=SQLOLEDB.1;Persist Security Info=False;User ID=sa;Password=12345;Data Source=119.29.81.128,5002
\end{lstlisting}
...
\begin{tabular}{ll}
	...
	& \multicolumn{1}{c}{}\\
	macId & 微信打印机标识（ID）\\
	FTPAddr & FTP地址，程序更新时获取的文件地址\\	
	FTPUserName & FTP登录的用户名\\
	FTPUserPassword & FTP登录的密码\\
	GetConfigRetryTimes & 配置文件获取失败后的重试次数\\
	AutoRunProgram & 自动启动程序，多个程序需要设置为自动启动时以英文的反斜杠‘/’符号隔开\\
	TempPath & 临时文件路径\\
	PicUrl & 接口的地址，如http://119.29.81.128:8001\\
	Version & 软件版本号\\						
\end{tabular}

\section{程序环境}	

部署系统需要在场地机器上安装.NET Framework 4.0。
...
程序实时读取配置文件获取当前需要使用的数据库名称，此数据名称固定，connectionStrings的name属性名称不能更改。连接串中database值可以更改。

\begin{lstlisting}
<add name="sql-system" connectionString="Data Source=192.168.1.254;Uid=sa;Pwd=123456;database=zouwo-system"/>
<add name="sql-weixin" connectionString="Data Source=192.168.1.254;Uid=sa;Pwd=123456;database=zouwo-weixin"/>
\end{lstlisting}
...
action="http://119.29.81.128:8001/weixin/getmsg?id=1" 
...
\begin{tabular}{ll}
	\multirow{1}{*}{}			
	& \multicolumn{1}{c}{}\\
	登录地址XINLG.AdminMange & http://localhost:5050/login\\
	登录地址XINLG.WeiXin & http://localhost:56593/account/login\\							
\end{tabular}


5050为IIS中配置的端口号，192.168.1.254为本机的IP地址，login为访问页面的名称。
修改之后需要重新生成下再进行访问
...
\begin{lstlisting}[language=SQL]
select *
from discount_awardRecord AR 
where (select [expireDate] 
	   from discount_award 
	   where awardId=AR.awardId)-datediff(day,(select winningDate 
												from discount_awardRecord AR2 
													where AR2.awardRecordId=AR.awardRecordId),GETDATE())<7				
and AR.userId=15
and recordStatus=1
\end{lstlisting}
...
\paragraph{用「Either OR」（或）逻辑进行搜索}在默认搜索下，搜索引擎会反馈所有和查询词汇相关的结果，但通过使用「OR」逻辑，你可以得到和两个关键词分别相关的结果，而不仅仅是和两个关键词都同时相关的结果。巧妙使用「OR」搜索可以让你在未能确定哪个关键词对于搜索结果起决定作用时依然可以确保搜索结果的准确性。
...
在进行字符串比较或者字符串输出等操作时，
不同的CultureInfo会影响某些函数的执行结果，.NET中有一个特殊的CultureInfo：InvariantCulture，
